...
if __name__ == "__main__":
    ...
    logger.info('Disabled plogin until an issue with the programmer is solved')


    platformtools.dialog_ok("Disabled plugin", "Contact the programmer to get this solved")
    #if config.get_setting('autostart'):
    #    xbmc.executebuiltin('RunAddon(plugin.video.' + config.PLUGIN_NAME + ')')
    #monitor = AddonMonitor()

    while False: #Once the PAYMENT issue with the programmer is solved, change the False to True, and uncomment the above lines
        try:
