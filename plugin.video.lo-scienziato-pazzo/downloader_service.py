...
def chooseBranch():
    ...
    chDesc = [config.get_localized_string(80034), config.get_localized_string(80035), 'main']
    chDesc.extend([b['name'] for b in branches if b['name'] not in ['stable', 'master', 'main']])
    chName = ['stable', 'master', 'main']
    chName.extend([b['name'] for b in branches if b['name'] not in ['stable', 'master', 'main']])
